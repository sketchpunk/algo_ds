...
// Very basic implementation of a Finite State Machine designed to only store components and transitioning between them.
// T is for any specific object type that may be needed to be passed to a Component during its transition.
export default class StateMachine<T> {
    ...
    register( compInstance: IStateMachine<T>, isDefault: boolean = false ): this {
      this.components.set( compInstance.name, compInstance );
      if( isDefault ) this.defaultName = compInstance.name;
      return this;
    }
    ...
    remove( name: string, obj: T ): boolean {
      if( this.components.has( name ) ){
        ...
        if( this.active != null && this.active.name === name ) this.deactivate( obj );
  
        // Remove default component if its the same one being removed
        if( this.defaultName === name ) this.defaultName = '';
    ...
    getActiveName(): string | null { return this.active?.name || null; }
  
    /** Get the active component for direct access to its methods */
    getActive(): IStateMachine<T> | null { return this.active; }
    // #endregion
